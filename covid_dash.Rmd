...
imp = data.table::fread('files/imp.csv')
...
```{r}
min_month = total[1:132] %>% select(date,value,new_deaths,daily_test) %>% 
  group_by(lubridate::month(date)) %>% 
  summarise_all(list(min)) %>% .[2:5]

names(min_month)[2:4]=paste('min',names(min_month)[2:4],sep='_')

max_month = total[1:132] %>% select(date,value,new_deaths,daily_test) %>% 
  group_by(lubridate::month(date)) %>% 
  summarise_all(list(max)) %>% .[2:5]

names(max_month)[2:4]=paste('max',names(max_month)[2:4],sep='_')

all_month = cbind(min_month[2:4],max_month) %>% select(date,everything()) 
all_month$date = seq(as.Date('2020-03-01'),as.Date('2020-07-01'),'month')
...
tot <- melt(total %>% select(date,value,new_deaths) %>% 
              rename(new_cases = value) %>% filter(date<='2020-07-20'), id="date")
  ...
  hc_xAxis(
    ...
    plotLines = list(
      list(label = list(text = "Test data",rotation=90, textAlign = 'left'),
           color = "#FCFFC5",
           width = 2,
           from = datetime_to_timestamp(as.Date('2020-07-14',tz='UTC')),
           to = datetime_to_timestamp(as.Date('2020-07-20',tz='UTC'))
      ),
      list(label = list(text = "Train data",rotation=90, textAlign = 'left'),
           color = "#ffcccc",
           width = 2,
           from = datetime_to_timestamp(as.Date('2020-03-11',tz='UTC')),
           to = datetime_to_timestamp(as.Date('2020-07-13',tz='UTC'))
      ),
      list(label = list(text = "Prediction",rotation=90, textAlign = 'left'),
           color = "#ccff99",
           width = 2,
           from = datetime_to_timestamp(as.Date('2020-07-21',tz='UTC')),
           to = datetime_to_timestamp(as.Date('2020-08-05',tz='UTC'))
      )
    )
  ) %>% 
